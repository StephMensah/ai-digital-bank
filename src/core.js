...
const FID = (() => {
  ...
  /* ---- one customer, one profile ------------------------------------------------------
     The bundled entities above are sample content for a page with no session. The moment
     someone signs in, their own data replaces it wholesale — name, accounts, history —
     rather than being patched over the top, which is how a new customer ended up looking
     at Ama's business. Anything not yet backed by the API is emptied rather than
  ...
  /* ------------------------------------------------------------- feature flags
     What this deployment has switched on, read once at boot. A feature that is
     off is absent from the screen rather than refused when tapped, and the
     server refuses it too — this only decides what is worth showing. */
  let features = {};
  const featuresReady = API === null ? Promise.resolve({}) :
    fetch(API + '/api/config')
      .then(r => r.ok ? r.json() : null)
      .then(b => { features = (b && b.features) || {}; return features; })
      .catch(() => ({}));

  /* Ghana mobile numbers, entered the way people say them: 024 123 4567.
     The canonical +233 form is what travels to the API, so the customer never
     has to know it exists. */
  const MOBILE_PREFIXES = ['20','23','24','25','26','27','28','50','53','54','55','56','57','59'];
  function normaliseMsisdn(input){
    if (!input) return null;
    const d = String(input).replace(/[^\d]/g, '');
    let local;
    if (d.length === 10 && d[0] === '0') local = d.slice(1);
    else if (d.length === 9) local = d;
    else if (d.length === 12 && d.startsWith('233')) local = d.slice(3);
    else if (d.length === 13 && d.startsWith('2330')) local = d.slice(4);
    else return null;
    return MOBILE_PREFIXES.includes(local.slice(0,2)) ? '+233' + local : null;
  }
  ...
  /** Open a brand new customer: creates the person, their account, and a session. */
  async function register({fullName, msisdn, email, password}){
  ...
  /** Move money in over the wallet or card rail. Returns the pending transaction. */
  async function topUp({accountId, amountMinor, method = 'mobile_money', msisdn}){
    if (!signedIn()) throw Object.assign(new Error('sign in required'), {code:'signed_out'});
    const res = await fetch(API + '/api/v1/payments/deposits', {
      method:'POST',
      headers:{'Content-Type':'application/json', 'Idempotency-Key': 'web-' + Date.now() + '-' + Math.random().toString(36).slice(2), ...authHeaders()},
      body: JSON.stringify({accountId, amountMinor, method, msisdn})
    });
    const json = await res.json().catch(() => ({}));
    if (!res.ok) throw Object.assign(new Error(json.error?.message || 'Top up failed'),
      {code: json.error?.code || 'topup_failed'});
    return json;
  }

  /** The id of the signed-in customer's first account, needed for top ups. */
  ...
  async function submitGhanaCard({idNumber, dateOfBirth}){
    const res = await fetch(API + '/api/v1/kyc/ghana-card', {
      ...
      body: JSON.stringify({idNumber, dateOfBirth})
    });
    const json = await res.json().catch(() => ({}));
    if (!res.ok) throw Object.assign(new Error(json.error?.message || 'Verification failed'),
      {code: json.error?.code || 'kyc_failed'});
    return json;
  }

  /** Who owns this destination, before any money moves. */
  async function nameEnquiry(body){
    const res = await fetch(API + '/api/v1/payments/name-enquiry', {
      method:'POST', headers:{'Content-Type':'application/json', ...authHeaders()},
      body: JSON.stringify(body)
    });
    const json = await res.json().catch(() => ({}));
    if (!res.ok) throw Object.assign(new Error(json.error?.message || 'We could not find that account'),
      {code: json.error?.code || 'enquiry_failed', status: res.status});
    return json;
  }

  const savePayee = (payee) =>
    fetch(API + '/api/v1/payees', {
      method:'POST', headers:{'Content-Type':'application/json', ...authHeaders()},
      body: JSON.stringify(payee)
    }).then(async r => {
      const j = await r.json().catch(() => ({}));
      if (!r.ok) throw Object.assign(new Error(j.error?.message || 'Could not save that payee'),
        {code: j.error?.code || 'save_failed'});
      return j.payee;
    });
  ...
  async function walletToBank({accountId, amountMinor, msisdn, destination, narration}){
    const res = await fetch(API + '/api/v1/payments/wallet-to-bank', {
      method:'POST',
      headers:{'Content-Type':'application/json',
        'Idempotency-Key':'w2b-' + Date.now() + '-' + Math.random().toString(36).slice(2), ...authHeaders()},
      body: JSON.stringify({accountId, amountMinor, msisdn, destination, narration})
    });
    const json = await res.json().catch(() => ({}));
    if (!res.ok) throw Object.assign(new Error(json.error?.message || 'That transfer did not go through'),
      {code: json.error?.code || 'w2b_failed'});
    return json;
  }

  const kycStatus = () =>
    fetch(API + '/api/v1/kyc/status', {headers: authHeaders()})
      .then(r => r.ok ? r.json() : {status:'unknown'}).catch(() => ({status:'unknown'}));
  ...
  async function apiCall(path, body){
    ...
    const json = await res.json().catch(() => ({}));
    /* The compat surface used to answer {error:'invalid_pin'} and now answers
       {error:{code,message}}. Read both, so the sentence reaches the screen. */
    if (!res.ok){
      const code = (json.error && json.error.code) || json.error || 'request_failed';
      const message = (json.error && json.error.message) || json.message || String(code).replace(/_/g,' ');
  ...
  /* ----------------------------------------------------------- remittance
     Behind the remittance flag. The quote is priced and held by the server; the
     client sends back its id, never a price of its own. */
  async function remitCorridors(){
    const res = await fetch(API + '/api/v1/remittance/corridors', {headers: authHeaders()});
    if (!res.ok) throw Object.assign(new Error('unavailable'), {code:'feature_disabled'});
    return (await res.json()).corridors;
  }
  const remitQuote = (corridor, amountMinor) =>
    apiCall('/api/v1/remittance/quotes', {corridor, amountMinor});
  async function remitSend(payload){
    if (!apiUp) throw Object.assign(new Error('offline'), {code:'offline'});
    const res = await fetch(API + '/api/v1/remittance/transfers', {
      method:'POST',
      headers:{'Content-Type':'application/json',
        'Idempotency-Key':'rmt-' + Date.now() + '-' + Math.random().toString(36).slice(2), ...authHeaders()},
      body: JSON.stringify(payload)});
    const json = await res.json().catch(() => ({}));
    if (!res.ok){
      const code = (json.error && json.error.code) || 'request_failed';
      throw Object.assign(new Error((json.error && json.error.message) || 'That did not go through'), {code});
    }
    return json;
  }
  ...
  return { CCY, money, money0, pct, seed, ENTITIES, LANG, FLOOR, get live(){ return apiUp; },
           ...
           featuresReady, get features(){ return features; },
           remitCorridors, remitQuote, remitSend,
